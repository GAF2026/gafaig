...
export async function getRegistryByRegistryId(registryId: string): Promise<RegistryRecord | null> {
  const normalizedRegistryId = String(registryId ?? "").trim();
  if (!normalizedRegistryId) return null;

  const rows = await sfQuery<any>(`
    ${baseRegistrySelectSql()}
    WHERE UPPER(TRIM(REGISTRY_ID)) = UPPER(TRIM('${escapeSqlString(normalizedRegistryId)}'))
    LIMIT 1
  `);
...
export async function getRegistryRecordByRegistryId(registryId: string): Promise<RegistryRecord | null> {
  return getRegistryByRegistryId(registryId);
}

export async function searchRegistryRecords(args: string | SearchRegistryArgs): Promise<RegistryRecord[]> {
  ...
  if (country) {
    whereClauses.push(`UPPER(TRIM(COUNTRY)) = UPPER(TRIM('${escapeSqlString(country)}'))`);
  }

  if (registryId) {
    whereClauses.push(`UPPER(TRIM(REGISTRY_ID)) = UPPER(TRIM('${escapeSqlString(registryId)}'))`);
  }

  if (caseId) {
    whereClauses.push(`UPPER(TRIM(CASE_ID)) = UPPER(TRIM('${escapeSqlString(caseId)}'))`);
  }

  if (applicationId) {
    whereClauses.push(`UPPER(TRIM(APPLICATION_ID)) = UPPER(TRIM('${escapeSqlString(applicationId)}'))`);
  }
