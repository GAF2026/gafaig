...
function mapRegistryAiSystemRow(r: any): RegistryAiSystem {
  return {
    ...
    // CRITICAL FIX: certification must come from CERTIFIED_* fields only
...
export async function getRegistryAiSystemsPaginated(
  ...
  const paginatedRows = mapped.slice(start, end);
...
export async function getRegistryAiSystemsSummaryStats() {
  ...
  return {
    totalSystems: Number(first.TOTAL_SYSTEMS ?? 0),
    linkedEntities: Number(first.LINKED_ENTITIES ?? 0),
