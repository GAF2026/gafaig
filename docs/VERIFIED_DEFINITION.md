...
- fail-closed
- messageString-based
- tied directly to a Snowflake-originated public certification record

Verification exists to prove that a published GAFAIG certification record is authentic, tamper-resistant, and independently verifiable using GAFAIG’s public verification key.
...
without recomputation of trust fields in:
- API
- UI
- SDK
- widget
...
All public trust surfaces must enforce published-only visibility.
...
Certification alone does not create public trust.

Approval alone does not create public trust.

Publication is required before a certification record may appear publicly.
...
- represents the finalized public trust outcome once published
...
- must not expose Application ID as a public trust label
- must not expose Case ID as a public trust label
...
Seed data is not production trust data.
