                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:justify-between">
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className={`rounded-full px-2.5 py-1 text-xs ring-1 ${toneForRisk(row.riskTier)}`}>
                        {row.riskTier ? `${row.riskTier} Risk` : "Risk Unknown"}
                      </span>

                      <span className={`rounded-full px-2.5 py-1 text-xs ring-1 ${toneForCertification(row.certificationStatus)}`}>
                        {row.certificationStatus || "Pending"}
                      </span>

                      {row.certifiedTier && (
                        <span className={`rounded-full px-2.5 py-1 text-xs ring-1 ${toneForTier(row.certifiedTier)}`}>
                          Tier {row.certifiedTier}
                        </span>
                      )}

                      {row.certifiedBand && (
                        <span className={`rounded-full px-2.5 py-1 text-xs ring-1 ${toneForBand(row.certifiedBand)}`}>
                    ...
                    <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-sm text-black/72">
                      <div><span className="font-medium text-black/50">Developer:</span> {row.developerOrganization || "—"}</div>
                      <div><span className="font-medium text-black/50">Country:</span> {row.country || "—"}</div>
                      <div><span className="font-medium text-black/50">Type:</span> {row.systemType || "—"}</div>
                      <div><span className="font-medium text-black/50">Deployment:</span> {row.deploymentStatus || "—"}</div>
                      <div><span className="font-medium text-black/50">Oversight:</span> {row.oversightLevel || "—"}</div>
                    </div>
                  </div>

                  <div className="flex flex-wrap gap-3">
                    {row.systemId && (
                      <PublicButtonLink href={`/registry/ai-systems/${row.systemId}`} variant="primary" size="sm">
                        View System
                      </PublicButtonLink>
                    )}

                    {row.registryId && (
                      <PublicButtonLink href={`/registry/${row.registryId}`} variant="secondary" size="sm">
                        View Registry
                      </PublicButtonLink>
                    )}
                  </div>
...
function Info({ label, value, breakAll = false }: { label: string; value: string; breakAll?: boolean }) {
  return (
    <div className="rounded-xl border border-black/5 px-3 py-3">
      <div className="text-[11px] uppercase tracking-[0.16em] text-black/55">{label}</div>
      <div className={`mt-2 text-[14px] text-black/85 ${breakAll ? "break-all" : ""}`}>
        {value}
      </div>
    </div>
  );
