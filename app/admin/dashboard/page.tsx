  ...
  async function loadRecentActivity() {
    ...
    try {
      const res = await fetch(
        `/api/admin/verification/events?caseId=${encodeURIComponent(activityCaseId)}`,
        { cache: "no-store" }
      );
      ...
      <main className="mx-auto max-w-[1100px] px-6 pt-14 pb-16">
        ...
        <section className="grid gap-4 md:grid-cols-3">
          <div className="rounded-2xl border border-black/10 p-5">
            ...
            <div className="mt-4 text-[52px] leading-none font-semibold text-black">
        ...
        <section className="mt-10 grid gap-4 lg:grid-cols-[1fr_1.2fr]">
          <div className="rounded-2xl border border-black/10 p-5">
            <h2 className="text-[16px] font-semibold text-black">Submissions by status</h2>
          ...
          <div className="rounded-2xl border border-black/10 p-5">
            <h2 className="text-[16px] font-semibold text-black">
              Recent activity <span className="text-black/50">(Case: {activityCaseId})</span>
            </h2>

            {eventsErr ? (
              <div className="mt-3 text-[14px] font-medium text-red-700">{eventsErr}</div>
            ) : null}
            ...
            <div className="mt-4 grid gap-3" aria-busy={eventsLoading ? "true" : "false"}>
              {eventsLoading ? (
                ...
                <div className="text-[14px] text-black/60">No recent events found.</div>
              ) : (
                events.map((e) => (
                  <div
                    key={e.eventId}
                  ...
                  >
                    ...
                    <div className="mt-2 flex items-center justify-between gap-3 flex-wrap text-[13px] text-black/65">
                      <span className="truncate">{e.actor || "—"}</span>
                      <span className="font-mono truncate">{e.eventId}</span>
                    </div>
                  </div>
                ))
              )}
            ...
            <div className="mt-4 flex justify-end">
              <a
                href={`/admin/verification/${encodeURIComponent(activityCaseId)}/events`}
        ...
        <footer className="mt-6 text-[13px] text-black/55">Data source: Snowflake</footer>
      </main>
    </div>
  );
}