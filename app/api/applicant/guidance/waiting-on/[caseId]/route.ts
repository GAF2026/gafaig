...
import {
  resolveWaitingOn,
} from "@/lib/guidance/waitingOnService";
...
/**
 * Authenticated read-only endpoint for deterministic Waiting-On Guidance.
 *
 * The endpoint:
 *
 * - resolves applicant identity and organization scope server-side;
 * - resolves case-scoped Repository Context;
 * - executes the registered Waiting-On Engine;
 * - returns Repository Context and Waiting-On execution traces;
 * - exposes participant-visible waiting explanations only;
 * - performs no ownership assignment or reassignment;
 * - performs no automatic waiting-condition resolution;
...
export async function GET(
  ...
  const response =
    await resolveWaitingOn({
      ...
      /*
       * Waiting-On rules require visibility into empty repository
       * categories so missing-record conditions remain deterministic.
       */
