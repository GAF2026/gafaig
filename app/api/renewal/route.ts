...
function makeId(prefix = "REN") {
  // Example: REN-20260130-<8hex>
  const yyyyMMdd =
...
async function safeReadStore(filePath: string): Promise<StoreShape> {
  try {
    ...
    if (parsed && typeof parsed === "object") {
      ...
      if (Array.isArray(anyParsed.renewals)) return { version: 1, lastUpdated: nowIso(), items: anyParsed.renewals };

      // Unknown object shape -> start clean
      return { version: 1, lastUpdated: nowIso(), items: [] };
    }
...
export async function POST(req: Request) {
  const filePath = path.join(process.cwd(), "app", "data", "renewals.json");
