...
async function getApplicantCase(
  ...
  try {
    const res = await fetch(
      `${baseUrl}/api/applicant/cases/${encodeURIComponent(caseId)}`,
      {
        cache: "no-store",
    ...
    const json = (await res.json()) as ApplicantCaseDetailResponse;

    if (res.ok && json.ok) return json;

    return {
      ok: false,
      error: json.error || `Case request failed with status ${res.status}`,
    };
  } catch (error) {
    return {
      ok: false,
...
export default async function ApplicantCaseDetailPage({
  ...
  const data = await getApplicantCase(caseId);
  ...
  const lifecycle = data.lifecycle || {
    ...
    decisionStatus: "NOT_READY_FOR_DECISION",
      ...
      <div className="space-y-7 sm:space-y-8">
        ...
        <section className="rounded-3xl border border-black/10 bg-white p-6 sm:p-8">
          ...
          <div className="mt-8 grid gap-4 sm:gap-5 md:grid-cols-3">
            <SummaryCard label="Case ID" value={applicantCase.caseId} />
            <SummaryCard label="Status" value={applicantCase.status} />
            <SummaryCard label="Stage" value={applicantCase.stage} />
            <SummaryCard label="Organization" value={applicantCase.organizationName} />
            <SummaryCard
              label="Contact Email"
              value={applicantCase.email || "No contact email"}
            />
            <SummaryCard
              label="Last Updated"
              value={applicantCase.updatedAt || "No recent update"}
            />
          </div>
        </section>
        ...
        <section className="rounded-3xl border border-black/10 bg-white p-6 sm:p-8">
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            <StatusLabel value={lifecycle.reviewStatus} />
            <StatusLabel value={lifecycle.decisionStatus} />
            <StatusLabel
              value={lifecycle.repositoryRecord ? "Repository Record" : "Workflow Only"}
            />
          </div>
        </section>
        ...
        <section className="rounded-3xl border border-black/10 bg-white p-6 sm:p-8">
          ...
          <div className="mt-8 grid gap-4 sm:gap-5 md:grid-cols-4">
            <MetricCard label="Repository Activity" value={counts.repositoryActivity} />
            <MetricCard label="Evidence Records" value={counts.uploadedEvidence} />
            <MetricCard label="Artifact Records" value={counts.artifacts} />
            <MetricCard label="Request Responses" value={counts.requestResponseRecords} />
            <MetricCard label="Remediation Records" value={counts.remediationRecords} />
            <MetricCard label="Certification Records" value={counts.certifications} />
            <MetricCard label="Open Requests" value={counts.openRequests} />
          </div>
        </section>
        ...
        <section className="rounded-3xl border border-black/10 bg-white p-6 sm:p-8">
          ...
          <div className="mt-8 grid gap-4 sm:gap-5 md:grid-cols-3">
            <WorkflowStep label="Submission Received" complete={workflow.submissionReceived} />
            <WorkflowStep label="Information Request" complete={workflow.informationRequest} />
            <WorkflowStep label="Deficiency Notice" complete={workflow.deficiencyNotice} />
            <WorkflowStep label="Response Submitted" complete={workflow.responseSubmitted} />
            <WorkflowStep label="Review" complete={workflow.review} />
            <WorkflowStep label="Certification" complete={workflow.certification} />
            <WorkflowStep label="Published" complete={workflow.published} />
          </div>
        </section>
